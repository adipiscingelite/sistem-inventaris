    ...
    if($jenis == "barang_masuk"){
      ...
      <table class="table">
        ...
        <tbody>
          ...
          include '../koneksi.php';
          $no=1;
          $data = $myConn->query("SELECT * FROM barang_masuk WHERE date(bm_tgl_masuk) >= '$tgl_dari' AND date(bm_tgl_masuk) <= '$tgl_sampai'");
     ...
     <table class="table">
      ...
      <tbody>
        ...
        include '../koneksi.php';
        $no=1;
        $data = $myConn->query("SELECT * FROM barang_keluar WHERE date(bk_tgl_keluar) >= '$tgl_dari' AND date(bk_tgl_keluar) <= '$tgl_sampai'");
        while($d = $data->fetch_array()){
          ?>
          <tr>
            <td><?php echo $no++; ?></td>
